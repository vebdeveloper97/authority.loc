...
/**
 * ToquvInstructionsController implements the CRUD actions for ToquvInstructions model.
 */
class ToquvInstructionsController extends BaseController
{
    /**
    ...
    public function actionIndex()
    {
        ...
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams, 'instruction', 1);
    ...
    {
        $searchModel = new ToquvOrdersSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams, 'instruction-model');
    ...
    public function actionView($id, $orderId = null)
    {
        ...
        $samo = Musteri::find()->select('id')->where(['token'=>'SAMO'])->orderBy(['id'=>SORT_DESC])->one()->id;
    ...
    public function actionCreate($id = null)
    {
        ...
        if(Yii::$app->request->isPost && !empty($data)){
            ...
            $modelTI->type = ToquvRawMaterials::MATO;
    ...
    public function actionUpdate($id, $orderId)
    {
        ...
        $modelInstruction = ToquvInstructions::findOne($id);
        $items = ToquvInstructions::getInstructionViewData($orderId, $id,'update');
        $rmItems = ToquvInstructions::getRawMaterials($id, $orderId, true);
        $pusFine = ToquvPusFine::find()->asArray()->all();
        $modelInstruction->cp['pus_fines'] = ArrayHelper::map($pusFine, 'id','name');
        $data = Yii::$app->request->post();
        ...
        if(Yii::$app->request->isPost && !empty($data)){
            ...
            if($modelTI->load($data) && $modelTI->save()){
                ...
                if(!empty($modelTI->toquvInstructionItems)){
                    foreach ($modelTI->toquvInstructionItems as $item){
                        $item->delete();
                    }
                }
                if(!empty($modelTI->toquvInstructionRms)){
                    foreach ($modelTI->toquvInstructionRms as $item){
                        $item->delete();
                    }
                }
                if(!empty($data['ItemsRM'])){
    ...
    public function actionPlusQuantity()
    {
        if (Yii::$app->request->isAjax) {
            ...
            $data = Yii::$app->request->post();
            if (!empty($data)) {
                $tro = ToquvRmOrder::findOne($data['tro_id']);
                $tir = ToquvInstructionRm::findOne($data['tir_id']);
                if($tro && $tir){
                    $tro->quantity += $data['qty'];
                    $tir->quantity += $data['qty'];
                    if($tro->save() && $tir->save()){
                        $response['status'] = 1;
                        $response['message'] = Yii::t('app', 'Saved Successfully');
                        $response['tir'] = $tir['quantity'];
                        $response['tro'] = $tro['quantity'];
                    }
                }
            }
            return $response;
    ...
    public function actionSaveAndFinish($id,$orderId){
            ...
            if($instruction !== null){
                if($instruction->status != 3){
                    ...
                    if($instruction->toquvInstructionRms){
                        ToquvInstructionRm::updateAll(['status' => 3], ['toquv_instruction_id' => $instruction->id]);
                    }
                    $instruction->save(false,['status']);
                }
                $response['status'] = 1;
                $response['message'] = "OK";
            }
        ...
        $instruction->status = 3;
        $instruction->save(false,['status','updated_at']);
    ...
    public function actionInstructionList(){
        ...
        $dataProvider = $searchModel->searchInstructionList(Yii::$app->request->queryParams);
