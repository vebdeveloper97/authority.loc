<?php
use yii\bootstrap\Collapse;use yii\helpers\Html;
use yii\helpers\Url;
        ...
        'items' => [
            [
                ...
                'content' => $this->render('search/_search_mato', [
                    ...
                    'entity_type' => null,
                    'search_mato' => null,
                    'url' => Url::to(['report-model-mato-sklad']),
                ]),
...
<div class="report-ip-title">
    <h3 class="text-center" style="padding-bottom: 25px;">
        ...
        <br>
        (<b><?=\app\modules\toquv\models\ToquvOrders::getOrderTypeList(\app\modules\toquv\models\ToquvOrders::ORDER_SAMO)?></b>)
    </h3>
</div>
                        ...
                        <tr style="<?= $bgStyle; ?>">
                            <?php if($model->group_by_ip){?>
                                <td class="text-center">
                                    ...
                                    <?php if($item['moi_id']){?>
                                    <span>
                                        <button type="button" class="btn btn-default btn-xs view-order" data-url="<?=Yii::$app->urlManager->createUrl(['toquv/toquv-orders/order-info','id'=>$item['tro_id']])?>"><i class="fa fa-eye"></i></button>
                                    </span>
                                    <?php }?>
                                </td>
                            <?php }?>
...
$js = <<< JS
    ...
    $("body").delegate(".report-table tbody tr","click",function(e) {
    ...
    $("body").delegate(".view-order","click",function(e) {
        $('#modal-report').modal('show');
        $('#modal-report').find('.modal-body').load($(this).attr('data-url'));
    });
