    ...
    <?= Collapse::widget([
        'items' => [
            [
                ...
                'content' => $this->render('search/_search_incoming_mato', ['model' => $model, 'data' => $data]),
...
<div class="report-ip-title">
    <h3 class="text-center" style="padding-bottom: 25px;">
        <?= Yii::t('app', "{name}: {from} - {to} sana oralig'idagi kirimlar miqdori", ['isOwn' => $labelIsOwn, 'from' => $data['from_date'], 'to' => $data['to_date'],'name' => $data['name']]) ?>
    </h3>
</div>

<table class="table table-bordered report-table" id="table-list">
    <thead>
    <tr>
        <th>№</th>
    ...
    if(!empty($items)){
        foreach ($items as $item):?>
            ...
            <tr style="<?= $bgStyle; ?>">
                ...
                <td><?= $item['from_dept'] ?> <?= $item['from_musteri'] ?></td>
                <td><?= $item['to_dept'] ?></td>
