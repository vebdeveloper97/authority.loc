    ...
    <?= Collapse::widget([
        'items' => [
            [
                ...
                'content' => $this->render('search/_search_outcoming', ['model' => $model, 'data' => $data]),
...
<div class="report-ip-title">
    <h3 class="text-center" style="padding-bottom: 25px;">
        <?= Yii::t('app', "{name}({isOwn}): {from} - {to} sana oralig'idagi chiqimlar miqdori", ['isOwn' => $labelIsOwn, 'from' => $data['from_date'], 'to' => $data['to_date'],'name' => $data['name']]) ?>
    </h3>
</div>
    ...
    if(!empty($items)){
        foreach ($items as $item):?>
            ...
            <tr style="<?= $bgStyle; ?>">
                ...
                <td><?= ($model->type==1)?$item['to_dept']:$item['musteri'] ?></td>
