    ...
    <?= Collapse::widget([
        'items' => [
            [
                ...
                'content' => $this->render('search/_search_service', ['model' => $model, 'data' => $data]),
...
<div class="report-ip-title">
    <h3 class="text-center" style="padding-bottom: 25px;">
        <?= Yii::t('app', "{name}: {from} - {to} sana oralig'idagi xizmat uchun yuborilgan iplar miqdori", ['from' => $data['from_date'], 'to' => $data['to_date'],'name' => $data['name']]) ?>
    </h3>
</div>
...
<table class="table table-bordered report-table">
    ...
    foreach ($items as $item):?>
        ...
        <tr style="<?= $bgStyle; ?>">
            ...
            <td><?= number_format((-1)*$item['count'], 3, '.', ' ') ?></td>
            <td><?= $item['from_dept'] ?></td>
            <td><?= $item['to_dept'] ?></td>
            <td style="width: 250px; word-spacing: normal;"><?= $item['add_info'] ?></td>
        </tr>
        <?php
        $count++;
    endforeach;
    ...
    <tr>
        ...
        <th></th>
        <th><?= number_format((-1)*$totalQty, 3, '.', ' '); ?></th>
        <th></th>
        <th></th>
        <th></th>
    </tr>
