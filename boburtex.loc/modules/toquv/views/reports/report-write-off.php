    ...
    <?= Collapse::widget([
        'items' => [
            [
                ...
                'content' => $this->render('search/_search_write_off', ['model' => $model, 'data' => $data]),
...
<div class="report-ip-title">
    <h3 class="text-center" style="padding-bottom: 25px;">
        <?= Yii::t('app', "{name}({isOwn}): {from} - {to} sana oralig'idagi hisobdan chiqarilgan miqdor", ['isOwn' => $labelIsOwn, 'from' => $data['from_date'], 'to' => $data['to_date'],'name' => $data['name']]) ?>
    </h3>
</div>
