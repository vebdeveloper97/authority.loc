        ...
        <div class="toquv-instructions-form" style="border: 1px solid #ccc;margin-top: 15px; padding: 5px;">
                ...
                <table class="table table-bordered table-middle">
                    ...
                    <?php $i = 1; foreach ($models as $key => $item){?>
                        <tr>
                            <td>
                                <?=$i?>
                                <input type="hidden" class="form-control" name="ToquvInstructionRm[<?=$key?>][model_orders_items_id]" value="<?=$item['model_orders_items_id']?>">
                                <input type="hidden" class="form-control" name="ToquvInstructionRm[<?=$key?>][toquv_rm_order_id]" value="<?=$item['toquv_raw_materials_id']?>">
                                <input type="hidden" class="form-control" name="ToquvInstructionRm[<?=$key?>][cp[quantity]" value="<?=$item['raw_fabric']?>">
                            </td>
                            <td>
                            ...
                            <td>
                                <?= $item->toquvRawMaterials->name." - <span class='material_".$item['model_orders_items_id']."'>". $item['raw_fabric']."</span> kg" ?>
                            </td>
                            <td>
                                <?= DatePicker::widget([
                                    ...
                                    'value' => $item->getMoiRel('TOQUV_MATO_SEH')['end_date'],
                                    'language' => 'ru',
                                    'pluginOptions' => [
                                        'format' => 'dd.mm.yyyy',
                                        'autoclose' => true,
                            ...
                            <td>
                                ...
                                echo  Html::dropDownList( "ToquvInstructionRm[{$key}][priority]",
                                    ...
                                    $item->priorityList,
                                    [
                                        'options' => $item->getPriorityList('options'),
                                        'class' => 'form-control',
            ...
            <div class="instruction-items-box" style="margin-top: 15px;">
                ...
                <table class="table table-bordered table-middle">
                    ...
                    $b = 0;?>
                        <?php foreach ($models as $key => $item){$kg = $item['raw_fabric'];?>
                            <?php foreach ($item->toquvRawMaterials->toquvRawMaterialIps as $m => $thread){?>
                            <tr class="tr_<?=$item['model_orders_items_id']?>">
                            ...
                            <td><?= $item->toquvRawMaterials->name." - <span class='material_".$key."'>". $kg."</span> kg" ?></td>
...
//                                    'value' => $models[$key->toquvInstruction->toquvInstructionItems[$b]->entity_id,
                                    'options' => [
                            ...
                            <td>
                                <?= Html::input('text', "ToquvInstructionRm[{$key}][child][{$m}][fact]", $percentage, ['class' => 'form-control number qty_'.$key,'percentage'=>$thread->percentage]);?>
                            </td>
                            <td>
                                <?= Html::textarea("ToquvInstructionRm[{$key}][child][{$m}][add_info]",'',['rows'=>1,'class'=>'form-control','style'=>'height:18px'])?>
                            </td>
                        </tr>
                        <?php $b++;}}?>
                    </tbody>
