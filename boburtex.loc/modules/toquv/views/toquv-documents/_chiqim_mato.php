...
$models = ($model->getTdi(ToquvDocuments::ENTITY_TYPE_MATO))?$model->getTdi(ToquvDocuments::ENTITY_TYPE_MATO):$models;
?>
<div class="row">
    <div class="col-md-3">
        ...
        'columns' => [
            ...
            [
                ...
                'value' => function($model){
                    if(!$model->isNewRecord&&$model->tib){
            ...
            [
                ...
                'value' => function($model){
                    ...
                    }elseif ($model->remain_roll){
                        return $model->remain_roll;
                    }
                },
                'options' => [
                    'disabled' => true,
            ...
            [
                ...
                'value' => function($model){
                    ...
                    }elseif ($model->remain_count){
                        return $model->remain_count;
                    }
                },
                'options' => [
                    'disabled' => true,
...
$this->registerJs("
    ...
    $('#documentitems_id').on('beforeDeleteRow', function (e, row) {
        let row_index = $(row).data('row-index');
        $('#ip_item'+row_index).remove();
        let tib = $(row).find('.tabularSelectEntity').val();
    });
");
...
<script>
    ...
    $('body').delegate('.entity_type','change' , function(e){
        let input = $(".list-cell__count").find('input');
        if($(this).val()==<?=$mato?>){
