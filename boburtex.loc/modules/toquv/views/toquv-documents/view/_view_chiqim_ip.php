...
<div class="toquv-documents-view">
    <div class="pull-right no-print" style="margin-bottom: 15px;">
        <?php if (Yii::$app->user->can('toquv-documents/chiqim_ip/update')): ?>
            <?php if($model->status != $model::STATUS_SAVED && $model->to_department != NULL):?>
                <?= Html::a(Yii::t('app', 'Update'), ["update", 'id' => $model->id,'slug' => $this->context->slug,'t' => $isOwn], ['class' => 'btn btn-primary']) ?>
                <?= Html::a(Yii::t('app', 'Save and finish'), ["save-and-finish", 'id' => $model->id, 'slug' => $this->context->slug, 'action' => 'VSP','t' => $isOwn], ['class' => 'btn btn-success']) ?>
            <?php endif;?>
        <?php endif;?>
        <?php if (Yii::$app->user->can('toquv-documents/chiqim_ip/delete')): ?>
            <?php if($model->status != $model::STATUS_SAVED && $model->to_department != NULL):?>
    ...
    <table class="table table-bordered table-responsive">
        <tr>
            ...
            <td><strong><?= Yii::t('app','Qayerga')?></strong>: <?= ($model->toDepartment->name)?$model->toDepartment->name:$model->musteri->name ?></td>
        </tr>
        <tr>
