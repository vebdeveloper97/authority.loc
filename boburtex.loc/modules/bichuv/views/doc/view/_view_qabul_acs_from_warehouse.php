...
<div class="toquv-documents-view">
    <div class="pull-right no-print" style="margin-bottom: 15px;">
        <?= Html::a('<span class="fa fa-arrow-left"></span>', ["index",'slug' => $this->context->slug], ['class' => 'btn btn-info']) ?>
        <?php if (Yii::$app->user->can("doc/{$this->context->slug}/update")): ?>
            <?php if($model->status != $model::STATUS_SAVED):?>
                <?= Html::a(Yii::t('app', 'Qabul qilish'), ["save-and-finish", 'id' => $model->id, 'slug' => $this->context->slug],
                    ['class' => 'btn btn-success']) ?>
                <?= Html::a('<i class="fa fa-fast-backward"></i> '.Yii::t('app', 'Return'), ["return-to-warehouse", 'id' => $model->id, 'slug' => $this->context->slug, 'type' => 'acs'],
                    ['class' => 'btn btn-danger']) ?>
            <?php endif;?>
        <?php endif;?>
        <?= Html::button('<span class="fa fa-print"></span>', ['class' => 'btn btn-primary print-btn']) ?>
    </div>
    <?php $items = $model->getAcceptedItems(); ?>
        ...
        <div class="center-text">
            <table class="table table-striped table-bordered">
                ...
                foreach ($items as $key => $item):?>
                    <tr>
                        <td><?= ($key+1) ?></td>
                        <td style="width:400px;"><?= "{$item['sku']}-{$item['name']}-{$item['property']}"?></td>
                        <td><?= $item['quantity'] ?></td>
