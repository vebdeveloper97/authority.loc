...
<div class="row">
    <div class="col-md-6">
        <?= $form->field($model, 'nastel_table_no')->dropDownList(
                \app\modules\mobile\models\MobileTables::getMobileTableByDepartmentMap(
                        \app\modules\hr\models\HrDepartments::getDepartmentIdByToken(
                                \app\models\Constants::$TOKEN_BICHUV)),
                ['id' => 'nastel_table_no','prompt' => '']); ?>
    </div>
    <div class="col-md-6">
        <?php if ($model->isNewRecord):?>
            <?= $form->field($model, 'nastel_table_worker')->widget(Select2::class, [
                'data' => []
            ]) ?>
        <?php else:?>
            <?= $form->field($model, 'nastel_table_worker')->widget(Select2::class, [
                'data' => \app\modules\mobile\models\MobileTablesRelHrEmployee::getResponsiblePersonMapList($model['nastel_table_no']),
            ]) ?>
        <?php endif;?>
    </div>
</div>
<div class="row">
    ...
    <div class="col-md-4">
        ...
        <h4 id="infoSize" style="display: none;"><small><?php echo Yii::t('app',"O'lcham o'zgartirish uchun o'lchamlar to'plamini tanlab, plyusni bosing")?></small></h4>
    </div>
</div>
<div>
    <?php
...
<div class="row">
    <div class="col-md-4">
        ...
        <?= $form->field($model, 'total_weight')->hiddenInput()->label(false); ?>
    </div>
    <div class="col-md-8">
...
<script>
    ...
    function calculateRm() {
        ...
        let rag = $('#<?= $rag; ?>');
        if (slice && totalQty && parseFloat(totalQty) > 0 && totalWeight) {
            rag.val((parseFloat(totalWeight) - parseFloat(slice)).toFixed(2));
            workWeight.val((parseFloat(slice) / parseFloat(totalQty) * 1000).toFixed(0));
        }
    }
    ...
    $('body').delegate('#barcodeInput', 'keyup', function (e) {
        ...
        if (e.which == 13) {
            ...
            $(this).val('').focus();
            let selectObj = $('#documentitems_id table tbody tr:last').find('input');
    ...
    /** Employee by mobile table **/
    $('#nastel_table_no').on('change',function (e) {
        let id = $(this).find('option:selected').val();
        $.ajax({
            url: '<?= $urlSelectedTableEmployee; ?>',
            type: 'POST',
            data: {id:id},
            success: function (response) {
                if (response.status == 1) {
                    $('#<?= $nastelTableWorker; ?>').html('');
                    var option = new Option(response.items.title, response.items.id);
                    $('#<?= $nastelTableWorker; ?>').append(option);
                }else{
