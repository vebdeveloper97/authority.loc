...
/* @var $property BichuvAcsProperties*/

$this->title = Yii::t('app', 'Bichuv Acs');
...
<div class="bichuv-acs-index">
    ...
    <div class="no-print" style="padding-bottom: 25px;">
        ...
        <?= Html::a('<i class="fa fa-file-excel-o"></i>',
            Url::to(['/bichuv/bichuv-acs/export-excel?type=index']), ['class' => 'btn btn-sm btn-info']) ?>
    </span>
    <span class="pull-right" style="padding-right: 10px;">
    ...
    <?php if (Yii::$app->user->can('bichuv-acs/create')): ?>
            <span class="pull-right" style="padding-right: 10px;">
            <?= Html::a('<span class="fa fa-plus"></span>', ['bichuv-acs/create'],
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                ...
                {
                    $id = $model->id;
                    $result = $model->showView($model->name, $id);
                    return $result;
                },
                'format' => 'html'
            ],
            ...
            [
                ...
                'value' => function($model){
                    return "<span class='imageView btn btn-primary' data-count='".count($model->bichuvAcsAttachments)."' src='".Url::to(['/bichuv/bichuv-acs/carousel','id'=>$model->id])
                        ."' style='font-weight:bold;border:1px solid;border-radius:20px;padding:1px 6px;margin-right:4px'>"
                        . count($model->bichuvAcsAttachments) ."</span>"
                        . Html::a('<i class="fa fa-image"></i>',
                            Url::to(['/bichuv/bichuv-acs/image','id'=>$model->id]),
                            ['id'=>'imageAction_'.$model->id,'class' => 'btn btn-sm btn-info imageAction',]);
                },
                'format' => 'raw',
                'options' => ['width' => '80px']
            ],
...
//                    if ($action === 'update') {
//                        return \yii\helpers\Url::to(['bichuv-acs/update','id' => $model->id]);
//                    }
//                    if ($action === 'delete') {
...
<?= \app\widgets\ModalWindow\ModalWindow::widget([
    'model' => 'bichuv-acs',
    'modal_id' => 'bichuv-acs-modal',
    'modal_header' => '<h3>'. Yii::t('app', 'Bichuv Acs') . '</h3>',
...
<?php
$js = <<< JS
$('body').delegate('#bichuvacs-sku', 'keyup', function(e) {
    if(e.keyCode == 13){
        let barcode = $.fn.generateBarcode($(this).val());
        $("#bichuv-acs-modal").find('#bichuvacs-barcode').val(barcode);
    }    
});

$("body").delegate("#creteBarcode", "click", function(e) {
        e.preventDefault();
        var barcode = $.fn.generateBarcode($("#bichuv-acs-modal").find('#bichuvacs-sku').val());
        $("#bichuv-acs-modal").find('#bichuvacs-barcode').val(barcode);
    });
