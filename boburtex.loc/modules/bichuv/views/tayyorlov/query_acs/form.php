...
\app\assets\ResponsiblePersonAsset::register($this);
    ...
    <div class="row">
        <div class="col-md-6">
            <?= $form->field($model, 'from_hr_employee')->widget(Select2::className(), [
                'data' => HrDepartmentResponsiblePerson::getResponsiblePersonMapList(HrDepartments::getDepartmentIdByToken(HrDepartments::TOKEN_TAYYORLOV))
            ]) ?>
        </div>
        <div class="col-md-6">
            <?=$form->field($model, 'to_hr_employee')->widget(Select2::className(), [
                'data' => $model->isNewRecord ? [] : HrDepartmentResponsiblePerson::getResponsiblePersonMapList($model->to_hr_department)
            ]);?>
        </div>
    </div>
...
?>
<?php
$url = Url::to(['/site/get-responsible-employee-by-department']);
$select2js = <<<JS
const getIncomingUrl = '{$url}'; //'/wms/wms-document/incoming_general_order/get-responsible-employee-by-department';

treeViewInputListener('#tree-to_department', '#bichuvdoc-to_hr_employee', getIncomingUrl);
treeViewInputListener('#tree-from_department', '#bichuvdoc-from_hr_employee', getIncomingUrl);


JS;

$this->registerJs($select2js);