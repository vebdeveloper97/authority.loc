...
?>
<div class="spare-item-view">
    <?php if(!Yii::$app->request->isAjax){?>
    <div class="pull-right" style="margin-bottom: 15px;">
        <?php if (Yii::$app->user->can('spare-item/update')): ?>
            <?php  if ($model->status != $model::STATUS_SAVED): ?>
                <?= Html::a('Update', ['update', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
            <?php endif; ?>
        <?php endif; ?>
        <?php if (Yii::$app->user->can('spare-item/delete')): ?>
            <?php  if ($model->status != $model::STATUS_SAVED): ?>
        ...
        'attributes' => [
            ...
            [
                ...
                'value' => function($model){
                    return (app\modules\bichuv\models\SpareItem::getStatusList($model->status))?app\modules\bichuv\models\SpareItem::getStatusList($model->status):$model->status;
                }
            ],
            [
