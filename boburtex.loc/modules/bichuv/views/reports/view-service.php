...
$this->title = Yii::t('app', '{doc_type}  №{number} - {date}', [
    ...
    'doc_type' => $model->getSlugLabel($slug)
]);
$this->params['breadcrumbs'][] = ['label' => Yii::t('app', '{doc_type}',
    ['doc_type' => $model->getSlugLabel($slug)]), 'url' => ["index-service", 'slug' => $slug]];
$this->params['breadcrumbs'][] = $this->title;
\yii\web\YiiAsset::register($this);
?>
<div class="toquv-documents-view">
    <div class="pull-right" style="margin-bottom: 15px;">
        <?=  Html::a(Yii::t('app', 'Back'), ["index-service",'slug' => $slug], ['class' => 'btn btn-info']) ?>
        <?php if (Yii::$app->user->can('doc/kochirish_mato/update')): ?>
            <?php if ($model->status != $model::STATUS_SAVED): ?>
                <?= Html::a(Yii::t('app', 'Update'), ["form-service", 'id' => $model->id, 'slug' => $slug, 't' => $t], ['class' => 'btn btn-primary']) ?>
                <?= Html::a(Yii::t('app', 'Save and finish'), ["save-and-finish", 'id' => $model->id, 'slug' => $slug, 't' => $t],
                    ['class' => 'btn btn-success']) ?>

                <?php if (Yii::$app->user->can('doc/kochirish_mato/delete')): ?>
                    <?= Html::a(Yii::t('app', 'Delete'), ["delete-service", 'id' => $model->id, 'slug' => $slug, 't' => $t], [
        ...
        <table class="table-bordered table">
            <tbody>
                <tr>
                    ...
                    <td><?= $model->modelList->article?></td>
                </tr>
                <tr>
                    <td class="text-bold"><?= Yii::t('app','Model Ranglari');?></td>
                    <td><?=$model->modelVar->colorPan->code." (".$model->modelVar->name.")"?></td>
                </tr>
            </tbody>
        </table>
