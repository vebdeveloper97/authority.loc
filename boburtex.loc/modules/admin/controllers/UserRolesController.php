...
class UserRolesController extends Controller
{
    ...
    {
        if(!Yii::$app->user->can('user-roles/'.Yii::$app->controller->action->id))
    ...
    {
        $check = Users::findOne(['user_role'=>$id]);
        if(Yii::$app->request->isAjax){
            Yii::$app->response->format = Response::FORMAT_JSON;
        }
        $response = [];
        ...
        if(empty($check)) {
            ...
            if(!Yii::$app->request->isAjax){
                Yii::$app->session->setFlash('error',Yii::t('app', 'Bu rol foydalanuvchiga biriktirilgan!'));
            }
            $response['message'] = Yii::t('app', 'Bu rol foydalanuvchiga biriktirilgan!');
        }
        if ( Yii::$app->request->isAjax ) {
            return $response;
        }
    ...
    public function actionExportExcel(){
        ...
        $filename = "user-roles_".date("d-m-Y-His").".xls";
        header('Content-Disposition: attachment;filename='.$filename .' ');
        header('Cache-Control: max-age=0');
        \moonland\phpexcel\Excel::export([
            'models' => UserRoles::find()->select([
                'id',
            ])->all(),
            'columns' => [
                'id',
