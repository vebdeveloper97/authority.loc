...
?>
<div class="size-collections-view">
    <?php if(!Yii::$app->request->isAjax){?>
    <div class="pull-right" style="margin-bottom: 15px;">
        <?php if (Yii::$app->user->can('size-collections/update')): ?>
            <?php  if ($model->status != $model::STATUS_SAVED): ?>
                <?= Html::a(Yii::t('app', 'Update'), ['update', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
            <?php endif; ?>
        <?php endif; ?>
        <?php if (Yii::$app->user->can('size-collections/delete')): ?>
            <?php  if ($model->status != $model::STATUS_SAVED): ?>
        ...
        'attributes' => [
            ...
            [
                ...
                'value' => function($model){
                    return (app\modules\base\models\SizeCollections::getStatusList($model->status))?app\modules\base\models\SizeCollections::getStatusList($model->status):$model->status;
                }
            ],
            'type',
            [
