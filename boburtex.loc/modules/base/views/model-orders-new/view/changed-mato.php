...
<div class="pull-right" style="margin-top: -22px;">
    <div class="pull-right" style="margin-top: -22px;">
        <?php if (P::can('model-orders/save-and-planned-toquv')): ?>
            <?php  if ($model->status == $model::STATUS_CHANGED_MATO): ?>
                <?= Html::a(Yii::t('app', 'Tasdiqlash'), ["save-and-checked-toquv", 'id' => $model->id], ['class' => 'btn btn-success']) ?>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</div>
<?php if (P::can('model-orders/save-and-planned-toquv')): ?>
    <?php  if ($model->status == $model::STATUS_CHANGED_MATO): ?>
    <form action="<?=\yii\helpers\Url::to(["cancelled-toquv", 'id' => $model->id])?>" method="post">
