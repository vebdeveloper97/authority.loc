...
<div class="model-planning">
    <?php
        $modelItems = $model->getModelOrdersItems()->where(['not', ['status' => $model::STATUS_INACTIVE]])->all()?true:false;
        $modelsItems = $model->getModelOrdersItems()->where(['not', ['status' => $model::STATUS_INACTIVE]])->all();
        if($modelItems):
    ?>
            <?php foreach ($modelsItems as $key => $item):?>
                    ...
                    <div class="col-md-8">
                        <div class="form-group">
                            ...
                            <div class="row parent_size_checkbox">
                                ...
                                <div class="col-md-9"><?=$item->getSizeCustomListPercentage('customDisabled alert-success','',$item->percentage,true,$item->getCheckedSizeList(MoiRelDept::TYPE_MATO_AKS))?></div>
                                <div class="col-md-1">
                ...
                <?php foreach ($item->modelOrdersItemsToquvAcs as $n => $m){
                    $mato = $m->getWmsMatoInfo()->where(['type' => \app\modules\toquv\models\ToquvRawMaterials::ACS])->all()?$m->getWmsMatoInfo()->where(['type' => \app\modules\toquv\models\ToquvRawMaterials::ACS])->all():false;
                    foreach ($mato as $k => $m) {
                    ?>
                    <div class="row planParent">
                        ...
                        <?=$form->field($models,'type')->hiddenInput([
                            ...
                            'class'=>'type_id',
                            'id' => "model_orders_type_{$key}_{$n}_{$key}"
                        ])->label(false)?>
                        ...
                        <div class="col-md-1">
                            <div class="list">
                                <?=Html::dropDownList('color_pantone', '', $item->modelVar->colorData, [
                                    'prompt' => Yii::t('app', 'Rang tanlang'),
                                    'class' => 'color_pantone',
                                    'value' => $m->wms_color_id,
                                ])?>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <?= $form->field($models, 'toquv_raw_materials_id',['template'=>"{input}<input type='text' value='{$m->toquvRawMaterials->name}' class='form-control plan_mato' disabled>"])->hiddenInput(['value'=>$m->toquv_raw_materials_id,"class"=>"plan_mato_id",
                            ...
                            <div class="row planParent">
                                ...
                                <div class="col-md-1">
                                    <?= $form->field($models, 'work_weight')
                                        ->textInput([
                                            'class'=>'number work_weight form-control',
                                            'id'=>'modelordersplanning-'.$key.'-work_weight_'.$n,'name'=>"ModelOrdersPlanning[{$key}][child][{$n}][work_weight]",
                                            'value'=>$m['work_weight'],
                                ...
                                <div class="col-md-1">
                                    <?= $form->field($models, 'raw_fabric')->textInput(['class'=>'number form-control','id'=>'modelordersplanning-'.$key.'-raw_fabric'.$n,'name'=>"ModelOrdersPlanning[{$key}][child][{$n}][raw_fabric]",'value'=>$m['raw_fabric']])->label(false)?>
                                </div>
                                <div class="col-md-1">
                                    <?= $form->field($models, 'count')->textInput(['class'=>'number required form-control','id'=>'modelordersplanning-'.$key.'-count'.$n,'name'=>"ModelOrdersPlanning[{$key}][child][{$n}][count]",'value'=>$m['count']])->label(false)?>
                                </div>
                                <div class="col-md-1">
...
$type_aks = \app\modules\base\models\MoiRelDept::TYPE_MATO_AKS;
$js = <<< JS
    ...
    if(size.length>0){
        ...
        mato_div.each(function() {
            ...
            '    <div class="col-md-1">' +
                        ...
                        '<input type="hidden" id="modelordersplanning-'+key+'-type-'+num+'" name="ModelOrdersPlanning['+key+'][child]['+num+'][type]" value="{$type_aks}">'+
            ...
            '                <select id="modelordersplanning-'+key+'-color_pantone_id-'+num+'" class="form-control color_pantone" name="ModelOrdersPlanning['+key+'][child]['+num+'][color_pantone_id]" aria-required="true">' +
                                color_pantone +
            '                </select>' +
            ...
            '            <input type="text" id="modelordersplanning-'+key+'-work_weight_'+num+'" class="number work_weight form-control" name="ModelOrdersPlanning['+key+'][child]['+num+'][work_weight]" from="from-'+key+'-work_weight" tofinish="modelordersplanning-'+key+'-finished_fabric'+num+'" toraw="modelordersplanning-'+key+'-raw_fabric'+num+'" aria-required="true" data-count="'+size_count+'">' +
            '            <div class="help-block"></div>' +
            '        </div>' +
            '    </div>' +
            '    <div class="col-md-1">' +
            '        <div class="form-group field-modelordersplanning-'+key+'-raw_fabric'+num+'">' +
            '            <input type="text" id="modelordersplanning-'+key+'-raw_fabric'+num+'" class="number form-control" name="ModelOrdersPlanning['+key+'][child]['+num+'][raw_fabric]" aria-required="true">' +
            '            <div class="help-block"></div>' +
            '        </div>' +
            '    </div>' +
            '    <div class="col-md-1">' +
            '        <div class="form-group field-modelordersplanning-'+key+'-count'+num+'">' +
            '            <input type="text" id="modelordersplanning-'+key+'-count'+num+'" class="number shart required form-control" name="ModelOrdersPlanning['+key+'][child]['+num+'][count]" aria-required="true">' +
            '            <div class="help-block"></div>' +
            '        </div>' +
            '    </div>' +
            '    <div class="col-md-1">' +
            ...
            '            <input type="text" id="modelordersplanning-'+key+'-finish_gramaj'+num+'" class="number form-control" name="ModelOrdersPlanning['+key+'][child]['+num+'][finish_gramaj]">' +
            '            <div class="help-block"></div>' +
            '        </div>' +
            '    </div>' +
            '    <div class="col-md-1">' +
...
$('body').delegate('.copyButton', 'click', function(e){
    ...
    let copy = '<div class="row planParent">' +
        ...
        '    <div class="col-md-1">' +
        '        <div class="form-group field-modelordersplanning-'+key+'-finish_gramaj'+num+'">' +
        '            <input type="text" id="modelordersplanning-'+key+'-finish_gramaj'+num+'" class="number form-control" name="ModelOrdersPlanning['+key+'][child]['+num+'][finish_gramaj]" value="'+finishGramaj+'">' +
        '            <div class="help-block"></div>' +
        '        </div>' +
        '    </div>' +
        '    <div class="col-md-1">' +
