            ...
            <div class="document-items">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <button type="button" class="btn btn-success btn-xs fromPlanningButton" title="<?php echo Yii::t('app','Plandan olish')?>" data-key="<?=$key?>"><i class="fa fa-plus"></i></button>
                    </div>
                ...
                <div class="parentDiv">
                    <?php foreach ($item->childDepts as $n => $m){?>
                        <div class="row planParent planParent_<?=$m->modelOrdersPlanning->id?>">
                            ...
                            <div class="col-md-1">
                                <?= $form->field($models, 'thread_length')->textInput(['class'=>'number form-control','value'=>$m['thread_length']??$m->modelOrdersPlanning->thread_length,'name'=>"MoiRelDept[{$key}][child][{$n}][thread_length]",])->label(false)?>
                            </div>
                            <div class="col-md-1">
                                <?= $form->field($models, 'finish_en')->textInput(['class'=>'number form-control','value'=>$m['finish_en']??$m->modelOrdersPlanning->finish_en,'name'=>"MoiRelDept[{$key}][child][{$n}][finish_en]",])->label(false)?>
                            </div>
                            <div class="col-md-1">
                                <?= $form->field($models, 'finish_gramaj')->textInput(['class'=>'number form-control','value'=>$m['finish_gramaj']??$m->modelOrdersPlanning->finish_gramaj,'name'=>"MoiRelDept[{$key}][child][{$n}][finish_gramaj]",])->label(false)?>
                            </div>
                            <div class="col-md-1">
                                <input type="text" disabled class="form-control" value="<?=$m->modelOrdersPlanning['raw_fabric']?>">
                            </div>
                            <div class="col-md-1">
                                <input type="hidden" name="MoiRelDept[<?=$key?>][child][<?=$n?>][model_orders_planning_id]" value="<?=$m->modelOrdersPlanning->id?>">
                                <?= $form->field($models, 'quantity')->textInput(['class'=>'number required form-control','id'=>'moireldept-'.$key.'-raw_fabric'.$n,'name'=>"MoiRelDept[{$key}][child][{$n}][quantity]",'value'=>$m['quantity']])->label(false)?>
                            </div>
                            <div class="col-md-1">
...
<?php ActiveForm::end()?>
<?php
$url_planning = \yii\helpers\Url::to('get-plan');
$js = <<< JS
...
$('body').delegate('.fromPlanningButton', 'click', function(e){
    let t = $(this);
    let parent = t.parents('.document-items');
    let parentDiv = parent.find('.parentDiv');
    let moi_id = parent.find('input[name$="[model_orders_items_id]"').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            if(response.status == 0){
                call_pnotify(response.message);
            }else{
                let plan = response.planning;
                if(plan){
                    plan.map(function (index) {
                        ...
                        let planCheck = parentDiv.find('.planParent_'+index.id);
                        if(planCheck.length==0){
                            let planParent = '<div class="row planParent planParent_'+index.id+'">' +
                            ...
                            '    </div>' +
                            '</div>';
                            parentDiv.append(planParent);
                        }else{
                            planCheck.find('.color_pantone').html(color_pantone);
                            planCheck.find('.color_boyoq').html(color_boyoq);
                            planCheck.find('.plan_mato').find('input').val(index.toquvRawMaterials.name);
                            planCheck.find('input[name$="[thread_length]"').val(index.thread_length);
                            planCheck.find('input[name$="[finish_en]"').val(index.finish_en);
                            planCheck.find('input[name$="[finish_gramaj]"').val(index.finish_gramaj);
                            planCheck.find('input[name$="[quantity]"').val(index.raw_fabric);
                            planCheck.find('input[name$="[add_info]"').val(index.add_info);
                        }
                    });
                }
            }
