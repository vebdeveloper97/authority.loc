...
$this->title = Yii::t('app', "To'quv Aksessuar Plan: {name}", [
        ...
        <?php foreach ($model->modelOrdersItems as $key => $item):?>
            <div class="document-items">
                ...
                <div class="row">
                    ...
                    <div class="col-md-1 mini-width">
                        <label><?=Yii::t('app','Plan(kg)')?></label>
                    </div>
                    <div class="col-md-1 mini-width">
                        <label><?=Yii::t('app','Plan(dona)')?></label>
                    </div>
                    <div class="col-md-1">
                ...
                <div class="parentDiv">
                    <?php foreach ($item->planningAks as $n => $m){
                            $dept = \app\modules\base\models\MoiRelDept::findOne(['model_orders_items_id'=>$item->id,'model_orders_planning_id'=>$m['id']]);
                    ?>
                        <div class="row planParent">
                            <div class="col-md-1 plan_size">
                                <input type="text" disabled class="form-control" value="<?=$m->size->name?>">
                                <input type="hidden" name="MoiRelDept[<?=$key?>][child][<?=$n?>][size_id]" value="<?=$m->size_id?>">
                            </div>
                            ...
                            <!--<div class="col-md-1 color_boyoq">
                                <?php /*$color = $m->color*/?>
                                <span style="background:  ;width: 10%">
                                    <span style="opacity: 0;">
                                        <span class="badge">
                                            r
                                        </span>
                            ...
                            <div class="col-md-1">
                                ...
                                <?= $form->field($models, 'quantity')->textInput(['class'=>'number required form-control','id'=>'moireldept-'.$key.'-raw_fabric'.$n,'name'=>"MoiRelDept[{$key}][child][{$n}][quantity]",'value'=>$dept['quantity']??$m['raw_fabric']])->label(false)?>
                            </div>
                            <div class="col-md-1">
                                <?= $form->field($models, 'count')->textInput(['class'=>'number form-control','id'=>'moireldept-'.$key.'-count'.$n,'name'=>"MoiRelDept[{$key}][child][{$n}][count]",'value'=>$dept['count']??$m['count']])->label(false)?>
                            </div>
                            <div class="col-md-1">
                                <?= $form->field($models, 'start_date')->widget(DatePicker::className(),[
                                    'options' => [
                                        ...
                                        'value' => $dept['start_date']??date('d.m.Y'),
                                    ],
                                    'language' => 'ru',
                                    'pickerButton' => false,
                            ...
                            <div class="col-md-1">
                                <?= $form->field($models, 'end_date')->widget(DatePicker::className(),[
                                    'options' => [
                                        ...
                                        'value' => $dept['end_date']??date('d.m.Y', strtotime('+7 days')),
                                    ],
                                    'language' => 'ru',
                                    'pickerButton' => false,
                                    'layout' => "{input}<span class='input-group-addon kv-date-remove'> <i class='fa fa-times kv-dp-icon'></i> </span>",
                            ...
                            <div class="col-md-1">
                                <?= $form->field($models, 'add_info')->textarea(['rows'=>'1','id'=>'moireldept-'.$key.'-add_info'.$n,'name'=>"MoiRelDept[{$key}][child][{$n}][add_info]",'value'=>$dept['add_info']??$m['add_info']])->label(false)?>
                            </div>
