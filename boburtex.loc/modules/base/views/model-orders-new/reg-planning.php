...
<div class="model-planning">
    <?php $modelsItems = $model->getModelOrdersItems()->where(['not', ['status' => $model::STATUS_INACTIVE]])->all()?true:false;
    $modelItems = $model->getModelOrdersItems()->where(['not', ['status' => $model::STATUS_INACTIVE]])->all();
    if($modelsItems):
    ?>
        <?php foreach ($modelItems as $key => $item):?>
            ...
            <div class="row">
                ...
                <div class="col-md-1 mini_width">
                    <label><?=Yii::t('app','Work Weight'); ?><span class="text-warning">(gr)</span></label>
                </div>
                <div class="col-md-1 mini_width">
            ...
            <?php foreach ($item->modelOrdersItemsMaterial as $n => $m){
                foreach($m->wmsMatoInfo as $k => $mato1){
                ?>
                    ...
                    <div class="col-md-1">
                        <div class="list">
                            <?=Html::dropDownList('color_pantone', '', $item->modelVar->colorData, [
                                'prompt' => Yii::t('app', 'Rang tanlang'),
                                'class' => 'color_pantone',
                                'value' => $mato1->wms_color_id,
                            ])?>
                        </div>
                    </div>
                    <div class="col-md-2 plan_mato_div">
                        <?= $form->field($models, 'toquv_raw_materials_id',['template'=>"{input}<input type='text' value='{$mato1->toquvRawMaterials->name}' class='form-control plan_mato' disabled>"])->hiddenInput(['value'=>$mato1->toquv_raw_materials_id,"class"=>"plan_mato_id",
                            ...
                            <div class="col-md-1 mini_width">
                                <?= $form->field($models, 'work_weight')
                                    ->textInput([
                                        ...
                                        'data-count'=> $item->getAllCountPercentage($item->percentage,$m['size_list'])?:$m['size_count']
                                    ])->label(false)?>
                            </div>
                            <div class="col-md-1 mini_width">
            ...
            <input type="hidden" class="num_size" value="<?=$num?>">
        </div>
        <?php endforeach;?>
    <?php endif; ?>
</div>
...
$js = <<< JS
$('body').delegate('.plan_mato', 'dblclick', function(e){
    let mato_div = $(this).parents('.document-items').find('.parentDivHidden').find('.plan_mato_div');
    let parent = $(this).parents('.form-group');
    let name = parent.find('.plan_mato_id').attr('name');
    let select = "<select class='form-control select_mato' name='"+name+"'></select>";
    parent.html(select);
    let select_input = parent.find('.select_mato');
    mato_div.each(function() {
        let option = new Option($(this).find('.plan_mato').val(),$(this).find('.plan_mato_id').val());
...
$('body').delegate('.work_weight','change keyup',function(e){
    ...
    toFinish.val((Math.round(from*t.val() / 1000)).toFixed(2));
    toRaw.val(Math.round((from*t.val()*1.1) / 1000).toFixed(2));
...
$('body').delegate('.saveButton', 'click', function(e){
    if(confirm("Siz rostdan ushbu qatorni saqlamoqchimisiz?")){
...
$('body').delegate('.add_row_size', 'click', function(e){
    ...
    if(size.length>0){
        ...
        mato_div.each(function() {
            let lastChild = parent_div.find('.planParent').last();
            let lastButton = lastChild.find('.copyButton');
            let num_size = parent.find('input.num_size');
            let num = (lastButton.length > 0)?(1*lastButton.attr('data-num'))+1:1*num_size.val()+1;
            ...
            let boyoq_service = $(this).find('.boyoq_service').html();
            let boyoq_service_id = $(this).find('.boyoq_service').attr('value');
            ...
            '            <input type="text" id="modelordersplanning-'+key+'-finish_gramaj' +
             ''+num+'" class="number shart required form-control" name="ModelOrdersPlanning['+key+'][child]['+num+'][finish_gramaj]">' +
            '            <div class="help-block"></div>' +
            '        </div>' +
            '    </div>' +
            ...
            $('#modelordersplanning-'+key+'-boyoq_service-'+num).find('option').removeAttr('data-select2-id');
            $('#modelordersplanning-'+key+'-boyoq_service-'+num).val(boyoq_service_id).trigger('change');
            $('#modelordersplanning-'+key+'-boyoq_service-'+num).find('optgroup').removeAttr('data-select2-id');*/
            ...
            if (jQuery('#modelordersplanning-'+key+'-boyoq_service-'+num).data('select2')) { jQuery('#modelordersplanning-'+key+'-boyoq_service-'+num).select2('destroy'); }
            jQuery.when(jQuery('#modelordersplanning-'+key+'-boyoq_service-'+num).select2(select2_service)).done(initS2Loading('modelordersplanning-'+key+'-boyoq_service-'+num,'s2options_service'));*/
        });
    }else{
...
$('body').delegate('.copyButton', 'click', function(e){
    ...
    let boyoq_service = planParent.find('.boyoq_service').html();
    let boyoq_service_id = planParent.find('.boyoq_service').attr('value');
...
$('body').delegate('.removeButton', 'click', function(e){
    if(confirm("Siz rostdan ham ushbu qatorni o'chirmoqchimisiz?")){
...
var s2options_service = {"themeCss":".select2-container--krajee","sizeCss":"","doReset":true,"doToggle":true,"doOrder":false};
window.select2_service = {
    "allowClear":true,
    "multiple" : true,
    "escapeMarkup":function (markup) { 
        return markup;
   ...
   "theme":"krajee","width":"100%","placeholder":"Bo\u0027yoq xizmatini tanlang","language":"ru"};
JS;
$this->registerJs($js,\yii\web\View::POS_READY);