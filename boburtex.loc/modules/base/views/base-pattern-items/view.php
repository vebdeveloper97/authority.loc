...
?>
<div class="base-pattern-items-view">
    <?php if(!Yii::$app->request->isAjax){?>
    <div class="pull-right" style="margin-bottom: 15px;">
        <?php if (Yii::$app->user->can('base-pattern-items/update')): ?>
            <?php  if ($model->status != $model::STATUS_SAVED): ?>
                <?= Html::a(Yii::t('app', 'Update'), ['update', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
            <?php endif; ?>
        <?php endif; ?>
        <?php if (Yii::$app->user->can('base-pattern-items/delete')): ?>
            <?php  if ($model->status != $model::STATUS_SAVED): ?>
    ...
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            [
                'attribute' => 'status',
                'value' => function($model){
                    return (app\modules\base\models\BasePatternItems::getStatusList($model->status))?app\modules\base\models\BasePatternItems::getStatusList($model->status):$model->status;
                }
            ],
            [
