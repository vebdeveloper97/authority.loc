                                    ...
                                    'addon' => [
                                        'append' => [
                                            'content' => KHtml::button(KHtml::icon('plus'), [
                                                'class' => 'showModalButton btn btn-success btn-sm model-var-id',
                                                'style' => 'width:15px; padding:2px; font-size: 8px',
                                                'title' => Yii::t('app', 'Create'),
                                                'value' => Url::to(['models-variations/create', 'id' => $models->models_list_id]),
                                                'data-toggle' => "modal",
                ...
                <tr>
                    <td colspan="16" style="margin: 0px;">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="col-md-12">
                                    <?= $form->field($models, 'sum_item_qty',['template'=>"<div><div class='col-md-6 text-right'>{label}</div><div class='col-md-6 noPaddingLeft'>{input}</div></div>"])->textInput(['id'=>'sum_item_qty']);?>
                                </div>
                                <div class="col-md-12">
                                ...
                                [
                                    ...
                                    'options' => [
                                        ...
                                        'addon' => [
                                            'append' => [
                                                'content' => KHtml::button(KHtml::icon('plus'), [
                                                    'class' => 'showModalButton btn btn-success btn-sm bichuv_acs_id',
                                                    'style' => 'width:15px; padding:2px; font-size: 8px',
                                                    'title' => Yii::t('app', 'Create'),
                                ...
                                [
                                    ...
                                    'options' => [
                                        ...
                                        'addon' => [
                                            'append' => [
                                                'content' => KHtml::button(KHtml::icon('plus'), [
                                                    'class' => 'showModalButton btn btn-success btn-sm model-toquv-acs',
                                                    'style' => 'width:15px; padding:2px; font-size: 8px',
                                                    'title' => Yii::t('app', 'Create'),
                    ...
                    .done(function(response) {
                        if (response.status) {
                            ...
                            $.ajax({
                                url: '<?=$model_variation?>',
                                type: 'GET',
                                data: {toquvId: response.model.toquv_raw_material_id, colId:response.model.wms_color_id, desId: response.model.wms_desen_id},
                                success:function(res){
                                    if(res.status){
                                        for(let i in res.data){
                                            let d = res.data[i];
                                            let color;
                                            if(d.color_pantone_id){
                                                color = d.cpcode+'('+d.cpname+')';
                                            }
                                            else{
                                                color = d.color_code+'('+d.color_name+')';
                                            }
                                            let Options = new Option(color, response.model.id, true, true);
                                            $('#modelordersitems-model_var_id').append(Options);
                                            $('#add_new_item_modal').modal('hide');
                                        }
                                    }
                                    else{
