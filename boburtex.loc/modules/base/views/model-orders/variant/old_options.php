...
/* @var $isOptions ModelOrdersVariations */

$materials = ModelOrdersVariations::getVariantMaterials($model->id, 2, $variant_id, $model->status);
$acs = ModelOrdersVariations::getVariationAcs($model->id,2, $variant_id, $model->status);
$comments = $model->getComments($variant_id);
?>
    <?php if($isOptions === null): ?>
        <div class="pull-right" style="margin-bottom: 15px;">
            <?=  Html::a("<i class='fa fa-file-movie-o'></i>", ["get-copy-save?id={$id}&var_id={$variant_id}"], ['class' => 'btn btn-info copy-save']) ?>
        </div>
    <?php endif; ?>
    ...
    <table class="table table-bordered">
        <thead>
            <th colspan="6"><?=Yii::t('app', 'Reason')?></th>
            <th colspan="6"><?=Yii::t('app', 'Comment')?></th>
        </thead>
        <tbody>
            <tr>
                <td colspan="6">
                    <?php foreach($comments['comment'] as $item): ?>
                        <p class="badge badge-warning" style="background: #ffa500"><?=$item['name'];?></p>
                    <?php endforeach; ?>
                </td>
                <td colspan="6">
                    <?php foreach($comments['comment_var_rel'] as $item): ?>
                        <p class="text-success"><?=$item['comment'];?></p>
                    <?php endforeach; ?>
                </td>
            </tr>
        </tbody>
    ...
    <table class="table table-bordered">
        <thead>
            <th colspan="12"><?=Yii::t('app', 'Images')?></th>
        </thead>
        <tbody>
            <tr>
