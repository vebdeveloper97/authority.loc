    ...
    <div class="row">
                            ...
                            success: function(response){
                                if(response.status){
                                    ...
                                    dataTypeId.map(function(val, k){
                                        let sum = parseInt(val.sum);   
                                        var newOption = new Option(val.doc_number +' ('+ val.musteri +')( Ish soni: '+sum+')(Sana: '+ formatDate(val.reg_date,'.') +')', val.id, false, false);
        ...
        <div class="col-md-6">
            <?= $form->field($model, 'order_id')->widget(Select2::classname(), [
                'data' => ($model->isNewRecord)?[]:$model->order_id,
                'size' => Select2::SIZE_SMALL,
                'options' => [
                        ...
                        $.ajax({
                            ...
                            success: function(response){
                                if(response.status){
                                    ...
                                    order.html('');
                                    dataTypeId.map(function(val, k){
                                        let sum = parseInt(val.summa);
                                        var newOption = new Option(val.artikul +' - '+ val.code +')(Ish soni: '+ sum +')' +' (Yuklama sanasi: '+ formatDate(val.load_date,'.') +')', val.id, false, false);
