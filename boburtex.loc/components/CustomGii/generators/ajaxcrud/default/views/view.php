...
if (($tableSchema = $generator->getTableSchema()) === false) {
    foreach ($generator->getColumnNames() as $name) {
        if($name != 'status' && $name != 'created_by' && $name != 'created_at' && $name != 'updated_at') {
            echo "            '" . $name . "',\n";
        }else{
            if($name=='status'){
                echo "[
                    ...
                    'value' => function(\$model){
                        return (<?= ltrim($generator->modelClass, '\\') ?>::getStatusList(\$model->status))?<?= ltrim($generator->modelClass, '\\') ?>::getStatusList(\$model->status):\$model->status;
                    }
                ],\n";
...
echo "            [
                'attribute' => '".$name."',
                'value' => function(\$model){
                    return (\app\models\Users::findOne(\$model->".$name."))?\app\models\Users::findOne(\$model->".$name.")->user_fio:\$model->".$name.";
                }
            ],\n";
...
echo "            [
                ...
                'value' => function(\$model){
                    return (".$modelName."::getStatusList(\$model->status))?".$modelName."::getStatusList(\$model->status):\$model->".$column->name.";
                }
            ],\n";
...
echo "            [
                'attribute' => '".$column->name."',
                'value' => function(\$model){
                    return (\app\models\Users::findOne(\$model->".$column->name."))?\app\models\Users::findOne(\$model->".$column->name.")->user_fio:\$model->".$column->name.";
                }
            ],\n";
