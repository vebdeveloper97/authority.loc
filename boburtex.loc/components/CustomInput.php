...
class CustomInput extends InputWidget
{
    /**
    ...
    /**
     * Datepicker with the input rendered as a appended bootstrap addon component
     */
    const TYPE_DEFAULT_INPUT = 7;
     ...
     */
    public $type = self::TYPE_DEFAULT_INPUT;
     ...
     * @inheritdoc
     */
    public $title_date = 'Select date';

    public $customLayout = '<button class="btn btn-success"><i class="fa fa-button"></i></button>';
    /**
     * @inheritdoc
     *
    ...
    protected function renderDatePicker()
    {
        ...
        $this->_hasAddon = $this->type == self::TYPE_COMPONENT_PREPEND || $this->type == self::TYPE_COMPONENT_APPEND || $this->type == self::TYPE_DEFAULT_INPUT;
        ...
        if (empty($this->layout)) {
            ...
            } elseif ($this->type === self::TYPE_DEFAULT_INPUT) {
                $this->layout = '{input}{picker}';
            }
        }
        ...
        if ($this->type !== self::TYPE_DEFAULT_INPUT&&$this->type !== self::TYPE_BUTTON&&$this->type !== self::TYPE_CUSTOM) {
            $this->registerAssets();
        }
        if($this->type == self::TYPE_CUSTOM){
            echo $this->customLayout;
        }else{
    ...
    {
        if ($this->type < 1 || $this->type > 8 || !is_int($this->type)) {
    ...
    protected function renderAddon(&$options, $type = 'picker')
    {
        ...
        if ($title !== false && empty($title)) {
            $options['title'] = $isPicker ? $this->title_date : Yii::t('kvdate', 'Clear field');
        }
    ...
    protected function parseMarkup($input)
    {
        ...
        switch ($this->type) {
            ...
            case self::TYPE_BUTTON:
                ...
                return Html::tag('span', "{$button}", $this->_container);
