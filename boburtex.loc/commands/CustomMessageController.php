...
use app\components\CustomFileHelper;
use Yii;
...
class CustomMessageController extends \yii\console\Controller
{
    public $ignoredPath = '@app/assets';
    /**
...
EOD;
    ...
    public function actionExtract($configFile = null)
    {
        ...
        $files = CustomFileHelper::findFiles(realpath($this->config['sourcePath']),realpath($this->config['ignoredPath']), $this->config);

//        echo var_dump($this->config['messagePath']);die;
        $messages = [];
...
EOD;
    ...
    protected function initConfig($configFile)
    {
        ...
        $this->config['ignoredPath'] = Yii::getAlias($this->config['ignoredPath']);
